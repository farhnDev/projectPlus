...
import FavoriteRestIdb from '../../DataSource/favorite-rest-idb';
...
const Detail = {
  ...
  async afterRender() {
    ...
    const rest = await TheMovieDbSource.detailMovie(url.id);
    const detail = rest.restaurant;
    const restaurantContainer = document.querySelector('#rest');

    restaurantContainer.innerHTML = createMovieDetailTemplate(rest);
